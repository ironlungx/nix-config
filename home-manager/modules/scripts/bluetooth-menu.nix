...
let
  cfg = config.myhm.bluetooth-menu;
in
{
  options.myhm.bluetooth-menu.enable = lib.mkEnableOption "bluetooth-menu";
  config.home.packages =
    ...
    lib.mkIf cfg.enable [
      ...
      (pkgs.writeShellScriptBin "bluetooth-menu" ''
        ...
        # dmenu-compatible command to pipe into. Extra arguments are passed through.
        DMENU_BLUETOOTH_LAUNCHER="''${DMENU_BLUETOOTH_LAUNCHER:-tofi}"
        run_dmenu() {
            ...
            if [[ "$DMENU_BLUETOOTH_LAUNCHER" == "tofi"* ]]; then
                sed 's/$/ /' | $DMENU_BLUETOOTH_LAUNCHER "''${dmenu_args[@]}" | sed 's/ $//'
            else
        ...
        if command_present systemctl; then
            ...
            case $? in
                3)
                    ...
                    ;;
                4)
                    error "Bluetooth daemon is not present" "bluez is missing"
                    exit 4
                    ;;
            esac
        fi
